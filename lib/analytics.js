...
export function trackEvent(eventName, params = {}) {
  ...
  // Clarity receives the event name only. Keep form values, contact details,
  // consultation content, and other visitor data out of session tags.
...
export function trackLeadGeneration({ formName, location, leadSource, initialLandingPage = '', referrerHost = '' }) {
  trackEvent('generate_lead', {
    form_name: formName,
    location: location,
    lead_source: leadSource,
    initial_landing_page: String(initialLandingPage || '').slice(0, 120),
    referrer_host: String(referrerHost || '').slice(0, 120),
  });
}
