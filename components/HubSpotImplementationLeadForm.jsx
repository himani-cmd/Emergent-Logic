...
export default function HubSpotImplementationLeadForm() {
  ...
  async function handleSubmit(event) {
    ...
    const qualificationContext = [
      ...
      `HubSpot implementation or rollout need: ${formData.project_summary.trim()}`,
    ].join('\n');
  ...
  if (status === 'success') {
    return (
      <div className="flex min-h-[30rem] flex-col justify-center border border-emerald-200 bg-emerald-50 p-7 sm:p-9" role="status">
        ...
        <h3 className="mt-5 text-2xl font-bold text-slate-950">Rollout review request received</h3>
        <p className="mt-3 leading-7 text-slate-700">
          Emergent Logic will review the current state and rollout need before responding. Scope, timing, access, and platform decisions are confirmed separately.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} onFocusCapture={trackFormStart} className="border border-slate-200 bg-white p-6 shadow-sm sm:p-8" aria-labelledby="hubspot-form-title">
      <p className="text-sm font-semibold text-indigo-700">HUBSPOT ROLLOUT REVIEW</p>
      <h3 id="hubspot-form-title" className="mt-2 text-2xl font-bold text-slate-950">Show us what is built and what still needs to work</h3>
      ...
      <button type="submit" disabled={status === 'submitting'} className="mt-6 inline-flex min-h-12 w-full items-center justify-center bg-[#4B37C8] px-5 py-3 font-semibold text-white transition-colors hover:bg-[#38289F] disabled:cursor-not-allowed disabled:opacity-70">
        {status === 'submitting' ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" aria-hidden="true" />Submitting request</> : 'Request a HubSpot rollout review'}
      </button>
